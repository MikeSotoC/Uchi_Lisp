...
31. Ejecutar `UCHI_REDES_PE` y verificar `uchi_redes.csv` con `AGUA/DESAGUE`, `BZ` y accesorios.
32. Ejecutar `UCHI_ROAD` y validar dibujo de secciones tipo en capa `UCHI_ROAD`.
33. Ejecutar `UCHI_EXPORT_LANDXML` y verificar `uchi_surface.xml` con caras `<Faces>`.
34. Ejecutar `UCHI_DIAG` y validar launcher/appdir en logs.
35. Revisar logs `[UCHI] Flujo topográfico base iniciado/completado`.
36. Forzar fallback: mover temporalmente módulos de A y dejar `UCHI_APP_DIR` a B.
37. Verificar que el sistema anuncia fallback usado y ruta final activa.
38. Restaurar módulos y repetir APPLOAD en A.
